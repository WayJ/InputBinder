...
public class StickyHeaderDecoration extends RecyclerView.ItemDecoration {
    private Map<Long, RecyclerViewHolder> mHeaderCache;
    ...
    public View findHeaderViewUnder(float x, float y) {
        for (RecyclerViewHolder holder : mHeaderCache.values()) {
    ...
    private RecyclerViewHolder getHeader(RecyclerView parent, int position) {
        final long key = mAdapter.getHeaderId(position);

        if (mHeaderCache.containsKey(key)) {
            return mHeaderCache.get(key);
        } else {
            final RecyclerViewHolder holder = mAdapter.onCreateHeaderViewHolder(parent);
